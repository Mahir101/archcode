...
fn run_eslint(cwd: &str) -> Vec<LintDiagnostic> {
    ...
    let mut diags = vec![];
    let Ok(files) = serde_json::from_slice::<serde_json::Value>(&out.stdout) else {
        return diags;
    };
...
fn run_checkstyle(cwd: &str) -> Vec<LintDiagnostic> {
    ...
    let err_re = regex::Regex::new(
        r#"line="(\d+)"[^>]*col="(\d+)"[^>]*severity="(\w+)"[^>]*message="([^"]+)""#,
    )
    .unwrap();
...
fn run_dotnet_analyze(cwd: &str) -> Vec<LintDiagnostic> {
    ...
    let re =
        regex::Regex::new(r"([^:\s]+\.cs)\((\d+),(\d+)\):\s*(error|warning)\s+(CS\w+):\s*(.+)")
            .unwrap();
    let mut diags = vec![];
...
impl LintStore {
    ...
    pub fn summary(&self) -> String {
        ...
        let errors: usize = self
            ...
            .count();
        format!(
            "Lint: {total} diagnostics ({errors} errors) across {} files",
